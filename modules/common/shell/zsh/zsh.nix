...

{
  programs.zsh = {
    ...
    completionInit = ''
      ...
      compdef __start_kubectl kj
      compdef __start_kubectl kjx
      compdef __start_kubectl ky
    ...
    initContent = ''
      ...
      function kgn() {
        k get nodes --no-headers -owide \
        | awk '{if ($2 =="Ready") {print "✅ " $1 " " $3 " " $8"_"$9} else {print "❌ " $1 " " $2 " " $3 " " $8"_"$9}}'
      }

      function kp() {
        local param req pod ns
        param=$\{*: -1\}
        req=$(k get po -A --no-headers \
            | fzf --height=80% --border=sharp --prompt="Pod > ") || return
      ...
      function kd() {
        local ns dep selector
        ns=$(kubectl get ns -o custom-columns=":metadata.name" \
            | fzf --prompt="Namespace> ") || return
        dep=$(kubectl -n "$ns" get deployments -o custom-columns=":metadata.name" --no-headers \
            | fzf --prompt="Deployment> " \
                   --preview="kubectl -n $\{ns\} get pods --selector app.kubernetes.io/name={1}" \
                   --preview-window='45%,border-sharp') || return
        selector=$(k -n "$ns" get deployments "$dep" -oyaml \
            | yq '.spec.selector.matchLabels."app.kubernetes.io/name"')
        stern -n "$ns" --selector app.kubernetes.io/name="$selector"
      }

      function kgsecd() {
        local secretData kubectlParams
        secretData=$\{*: -1\}
        kubectlParams=$\{*:1:-1\}
